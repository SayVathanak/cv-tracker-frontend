...
import {
  ...
  FaFilePdf,
  FaCopy,
  FaRobot, // Added Robot icon for AI context
} from "react-icons/fa";
...
const CandidateCard = memo(
  ({
    ...
    return (
      <div
        onClick={() => !isProcessing && handleCardClick(person)}
        className={`group relative p-3 rounded border cursor-pointer overflow-hidden transform-gpu transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] 
      ${
        isProcessing
          ? "bg-white border-blue-200 shadow-[0_0_15px_rgba(59,130,246,0.1)] cursor-progress" // Active Blue Glow
          : isExpanded
      ...
      ${
        isSelected && selectMode
          ? "ring-1 ring-blue-500 border-blue-500 bg-blue-50/10"
          : ""
      }`}
      >
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-2.5 w-full">
            ...
            <div className="relative w-8 h-8 flex-none">
              <button
                ...
                } ${isProcessing ? "cursor-not-allowed opacity-0" : ""}`}
              >
                <FaCheck
                  size={12}
              ...
              <div
                className={`absolute inset-0 w-full h-full flex items-center justify-center text-xs font-bold border rounded transition-all duration-300 transform ${
                  !selectMode
                    ? "opacity-100 rotate-0 scale-100"
              ...
              ${
                isProcessing ? "border-blue-100 bg-blue-50 text-blue-600" : ""
              }`}
              >
                {isProcessing ? (
            ...
            <div className="overflow-hidden flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                {isProcessing ? (
                  // --- PROCESSING STATE (Active Scanner Look) ---
                  <div className="w-full pr-2">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest flex items-center gap-1">
                        <FaRobot size={10} className="animate-bounce" /> AI
                        Extracting...
                      </span>
                    </div>
                    {/* Animated Progress Bar */}
                    <div className="h-1 w-full bg-zinc-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full w-1/3 animate-[shimmer_1.5s_infinite_linear] origin-left"></div>
                    </div>
                  </div>
                ) : (
                  // --- READY STATE ---
                  <>
                    <h3 className="text-xs font-semibold text-black uppercase truncate leading-none transition-colors">
                      {person.Name}
                    </h3>
                    {person.status === "Ready" && (
              ...
              {!isProcessing &&
                person.Position &&
                person.Position !== "N/A" && (
                  <p className="text-[10px] font-bold text-blue-600 truncate tracking-wider mt-0.5 uppercase">
                    {person.Position}
                  </p>
                )}
          ...
          {!selectMode && (
            <div
              className={`flex gap-1 z-20 transition-all duration-300 ease-out pl-2 ${
                isExpanded || isProcessing
                  ? "opacity-100 translate-y-0"
                  : "opacity-0 translate-y-2 pointer-events-none lg:pointer-events-auto lg:group-hover:opacity-100 lg:group-hover:translate-y-0"
              }`} 
            >
              <button
                disabled={isProcessing}
              ...
              <button
                ...
                className={`p-2 text-zinc-400 hover:text-black bg-white border border-zinc-100 hover:border-black rounded transition-colors duration-200 ${
                  isProcessing ? "opacity-30 cursor-not-allowed" : ""
                }`}
              >
                <FaEdit size={10} />
              ...
              <button
                ...
                className="p-2 text-zinc-400 hover:text-red-600 bg-white border border-zinc-100 hover:border-red-500 rounded transition-colors duration-200"
              ...
              <button
                ...
                disabled={isProcessing}
                className={`p-2 text-blue-400 hover:text-blue-600 bg-white border border-zinc-100 hover:border-blue-500 rounded transition-colors duration-200 ${
        ...
        {/* SKELETON LOADER FOR DETAILS (Only shows when processing) */}
        <div className="space-y-1.5">
          ...
          <div
            className={`grid transition-all duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
              isExpanded && !isProcessing
                ? "grid-rows-[1fr] opacity-100 mt-2 border-t border-zinc-200 pt-2"
                : "grid-rows-[0fr] opacity-0 mt-0 border-t-0 pt-0"
            }`}
          >
            <div className="overflow-hidden min-h-0">
              ...
              <div className="flex gap-2 pb-1">
                <button
                  ...
                  className="flex-1 lg:hidden h-8 bg-white text-black text-xs font-bold uppercase tracking-wider rounded border border-black flex items-center justify-center gap-2 hover:bg-zinc-100 transition-colors"
                >
                  <FaFilePdf size={12} /> View Doc
                </button>
                <button
                  onClick={(e) => handleCopy(person, e)}
                  className={`flex-2 h-8 px-3 text-xs font-bold uppercase tracking-wider rounded border transition-all duration-200 flex items-center justify-center gap-2 w-full ${
                    copiedId === person._id
        ...
        {/* SHIMMER ANIMATION KEYFRAMES (Add this to your index.css if not present, or rely on animate-pulse) */}
        <style>{`
        @keyframes shimmer {
