...
const ControlPanel = ({
  ...
  handleFileChange, handleMoveCandidates, handleUpload, handleClearFiles, setSearchTerm, setSortOption, setSelectMode,
      ...
      <div className="flex gap-2 h-9">
        <div className="relative flex-1 h-full">
          ...
          >
            {files.length > 0 ? (
              <>
                <FaCheck className="text-green-500" /> {files.length} File(s)
                Ready
              </>
            ) : (
              <>
                <FaCloudUploadAlt className="text-md" /> UPLOAD CV{" "}
        ...
        {files.length > 0 && !isUploading && (
          <>
            <button
              ...
              className="px-4 bg-white border border-green-500 text-green-500 rounded hover:bg-green-50 transition shadow-sm"
            >
              <FaPlay size={12} />
            </button>
            <button
              onClick={handleClearFiles}
              className="px-4 bg-white border border-zinc-200 hover:border-red-300 hover:bg-red-50 hover:text-red-500 rounded text-zinc-500 transition"
            >
              <FaTrash size={12} />
            </button>
          </>
              ...
              <div className="h-1.5 w-full bg-zinc-200 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={`h-full rounded-full ${
                    uploadProgress >= 90 ? "bg-green-500" : "bg-black"
                  }`}
                />
              </div>
            </div>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-8 pr-2 h-8 bg-white border border-zinc-200 rounded text-[10px] md:text-xs focus:border-black focus:ring-1 focus:ring-black outline-none transition placeholder:text-zinc-400"
            value={searchTerm}
        ...
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
          ...
          <button
            ...
            className={`h-8 text-xs font-bold uppercase rounded border transition flex items-center justify-center gap-1 ${
              selectMode
                ? "w-8 bg-zinc-100 text-zinc-600 border-zinc-300 hover:bg-zinc-200 shrink-0"
          ...
          {/* Selection Actions (Only visible in Select Mode) */}
          {selectMode && (
            <>
              {/* All / None Toggle */}
              <button
                onClick={toggleSelectAll}
                className="px-2 h-8 text-xs font-bold uppercase rounded border border-zinc-200 hover:border-black transition truncate bg-white text-black shrink-0"
              >
                {isPageFullySelected ? "None" : "All"}
              </button>

              {/* NEW: Move Button (Blue) */}
              <button
                onClick={handleMoveCandidates}
                disabled={!isSelectionActive || !isAuthenticated}
                className={`flex-1 h-8 px-2 text-white text-xs font-bold uppercase rounded transition border flex items-center justify-center gap-1.5 min-w-[60px] ${
              ...
              <button
                ...
                disabled={!isSelectionActive || !isAuthenticated}
                className={`h-8 px-3 text-white text-xs font-bold uppercase rounded transition border shrink-0 ${
                  isSelectionActive && isAuthenticated
