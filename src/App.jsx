...
function App() {
  ...
  const handleDeleteFolder = async (id) => {
    try {
      ...
      Toast.fire({ icon: "error", title: "Could not delete folder" });
    }
  };
  ...
  const fetchCandidates = async (
    ...
    if (!isAuthenticated && !localStorage.getItem("cv_token")) return;
  ...
  const processedCandidates = candidates
    .filter((person) => {
      ...
      if (filters.position && person.Position !== filters.position)
        return false;
      if (filters.education && person.School !== filters.education)
        return false;
  ...
  const toggleSelectAll = useCallback(() => {
    ...
    const isPageSelected = currentPageIds.every((id) =>
  ...
  const handleFileChange = (e) => {
    ...
    setFiles((prevFiles) => {
      ...
      const uniqueFiles = newFilesArray.filter(
      ...
      if (uniqueFiles.length < newFilesArray.length) {
        Toast.fire({
          ...
          text: `${
            newFilesArray.length - uniqueFiles.length
          } file(s) are already in the list.`,
        });
      }
  ...
  const handleRetry = useCallback(
    async (person, e) => {
      ...
      try {
        const res = await axios.post(
          `${API_URL}/candidates/${person._id}/retry`
        );
    ...
    [editingCandidate, currentPage, searchTerm, activeFolder]
  );
    ...
    try {
      ...
      if (totalPages > 1) {
        Toast.fire({
          icon: "info",
          title: "Downloading...",
          text: `Fetching ${totalItems} items...`,
        });
        const promises = [];
        for (let p = 2; p <= totalPages; p++) {
          const nextParams = { ...params, page: p };
  ...
  const handleExport = async () => {
    ...
    Toast.fire({
      icon: "success",
      title: "Success",
      text: `Exported ${finalData.length} records.`,
    });
  };
  ...
  const saveEdit = async () => {
    try {
      await axios.put(
        `${API_URL}/candidates/${editingCandidate._id}`,
        editingCandidate
      );
      setEditingCandidate(null);
      fetchCandidates(currentPage, searchTerm, activeFolder);
  ...
  const handleBulkCopy = async (mode) => {
    if (mode === "selected") {
      const candidatesToCopy = processedCandidates.filter((c) =>
      ...
      await navigator.clipboard.writeText(
        formatCandidateText(candidatesToCopy)
      );
    ...
    >
      <AnimatePresence>
        {isDragging && (
          ...
          >
            ...
            <p className="text-2xl font-bold text-blue-600 uppercase tracking-widest">
              Drop Files Here
            </p>
          </motion.div>
        )}
      ...
      <Navbar
        ...
        currentUser={
          ...
          appSettings.profile.displayName.trim() !== ""
            ? appSettings.profile.displayName
            : currentUser
        }
      ...
      <main className="flex-1 flex overflow-hidden max-w-[1920px] mx-auto w-full relative">
        ...
        >
          ...
          <ControlPanel
            ...
            handleMoveCandidates={handleMoveCandidates}
          ...
          <div className="flex-none p-4 border-t border-zinc-200 bg-zinc-50 flex justify-between items-center z-10">
            <button
              onClick={() =>
                fetchCandidates(currentPage - 1, searchTerm, activeFolder)
              }
        ...
        <div
          className={`flex-1 bg-zinc-100 relative flex flex-col h-full overflow-hidden
            ${
              showMobilePreview
          ...
          {showMobilePreview && selectedPerson && !editingCandidate && (
            <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-zinc-200 p-4 shadow-[0_-5px_30px_rgba(0,0,0,0.1)] z-50 lg:hidden">
              <div className="flex gap-2">
                <a
                  href={`tel:${selectedPerson.Tel}`}
                  className="flex-1 py-3 bg-zinc-100 border border-zinc-200 rounded text-center flex justify-center items-center"
                >
                  <FaPhoneAlt className="text-zinc-600" />
                </a>
                <button
                  onClick={() => handleCopy(selectedPerson)}
                  className="flex-2 py-3 bg-black text-white rounded font-bold uppercase text-xs shadow-lg"
                >
                  {copiedId === selectedPerson._id ? "COPIED!" : "COPY DATA"}
                </button>
                <button
                  onClick={(e) => startEditing(selectedPerson, e)}
                  className="flex-1 py-3 bg-zinc-100 border border-zinc-200 rounded text-center flex justify-center items-center"
                >
